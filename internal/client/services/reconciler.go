...
func NewReconciler(
	...
	*alpha1.Service,
	platform.CreateServiceGroupRequest,
	platform.CreateServiceGroupResponse,
] {
	return &controller.Reconciler[
		*alpha1.Service,
		...
		Client:         client,
		Scheme:         scheme,
		ResourceClient: serviceClient,
		NewObject:      newObject,
		GetSpec:        getSpec,
		SetStatus:      setStatus,
		Logger:         ctrl.Log.WithName("serviceReconciler"),
	}
}

func newObject() *alpha1.Service {
	return &alpha1.Service{}
}

func getSpec(obj *alpha1.Service) *platform.CreateServiceGroupRequest {
	return &obj.Spec
}

func setStatus(obj *alpha1.Service, status *platform.CreateServiceGroupResponse) {
	if status == nil {
		return
	}
	obj.Status = *status

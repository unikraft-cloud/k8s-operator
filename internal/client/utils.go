...
// Convert converts between struct types by matching field names and types.
// It performs a best-effort mapping, copying values from the source struct to the
// destination struct where field names and compatible types align.
//
// Used for converting between CRD structs to go-sdk requests/responses.
func Convert[S any, T any](spec S) (T, error) {
	var req T

	srcVal := reflect.ValueOf(spec)
	if !srcVal.IsValid() {
		return req, fmt.Errorf("failed to copy spec to request: source is invalid")
	}

	if srcVal.Kind() == reflect.Ptr {
		if srcVal.IsNil() {
			return req, fmt.Errorf("failed to copy spec to request: source pointer is nil")
		}
	} else {
	...
	if err := copier.CopyWithOption(dst, srcVal.Interface(), copier.Option{
		...
		return req, fmt.Errorf("failed to copy spec to request: %w", err)
	}
